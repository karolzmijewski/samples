...
# SOLUTION
# runtime: 3ms, beats 88.19%
# memory: 19.61 MB, beats 16.39%
class Solution:
    def canFinish(self, numCourses: int, prerequisites: list[list[int]]) -> bool:
        graph = { }
        # it is crucial to use set, due to overall performance.
        # A set in Python is a hash table under the hood.
        # To check x in my_set, Python computes the hash of x and jumps directly to
        # the bucket where x would be stored.
        # That’s O(1) average time (constant, regardless of set size).
        # Contrary to list where the same operation (x in container) is O(n)
        visited = set() 
        visiting = set()
        def cycle_detected(node, nlist):
            visiting.add(node)
            for nn in nlist:
                if nn == node:
                    return True
                    ...
                    continue # already processed sucessfully
                if nn in graph:
                    if cycle_detected(nn, graph[nn]):
                        return True
        ...
        for course in range(numCourses):
            if course in graph:
                if cycle_detected(course, graph[course]):
                    return False
        return True
